...
app.get("/api/booking-history", async (req, res) => {
  ...
  try {
    ...
    if (results.length === 0) {
      return res
        .status(404)
        .json({ error: "No booking history found for this user" });
    }
...
app.delete("/api/bookings/:bookingId", async (req, res) => {
  ...
  try {
    ...
    const [bookingDetails] = await connection.query(
      "SELECT room_id, time_slot FROM bookings WHERE booking_id = ?",
      [bookingId]
    );
    ...
    const updateRoomStatus = await connection.query(
      `UPDATE rooms SET ${statusField} = 'Free' WHERE room_id = ?`,
      [booking.room_id]
    );
    ...
    res.json({
      message: "Booking successfully cancelled and room status updated.",
    });
  } catch (error) {
    if (connection) {
    ...
    res
      .status(500)
      .json({
        error: "Database error during the cancellation process",
...
app.get("/api/rooms", async (req, res) => {
  ...
  try {
    ...
    const [rows, fields] = await db.query(
      "SELECT room_id, room_name, details, img, status_8_10, status_10_12, status_13_15, status_15_17 FROM rooms WHERE enabled = 1"
    );
  ...
  try {
    ...
    if (approvedHistoryBookings.length > 0) {
      ...
      return res
        ...
        .json({
          error:
            "You have already an approved booking for today in booking history.",
        });
    }
    ...
    if (existingBookings.length > 0) {
      ...
      return res
        .status(409)
        .json({ error: "You already have a pending booking for today." });
    }
    ...
    if (conflictingBookings.length > 0) {
      ...
      return res
        .status(409)
        .json({ error: "Another booking already exists for this time slot." });
    }
    ...
    res
      .status(500)
      .json({
        error: "Database error during the booking process",
        details: error.message,
      });
  }
});
...
app.post("/api/approve-booking", async (req, res) => {
  ...
  try {
    ...
    await connection.query("DELETE FROM bookings WHERE booking_id = ?", [
      bookingId,
    ]);
...
app.post("/api/reject-booking", async (req, res) => {
  ...
  if (!bookingId || !approverId) {
    return res
      .status(400)
      .json({ error: "Booking ID and approver ID are required" });
  }
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to reject booking", details: error.message });
  } finally {
    connection.release();
  }
});
...
app.post("/Register1", async (req, res) => {
  ...
  try {
    ...
    res.json({
      user: { id: result[0].insertId, username: username },
      redirect: "/HomepageUser.html",
    });
  } catch (error) {
...
app.post("/login", async (req, res) => {
  ...
  try {
    ...
    if (user) {
      ...
      res
        .status(401)
        .send("Login failed: User not found or password incorrect");
    }
  } catch (error) {
...
// ========================= ADMIN Route =======================================
// API endpoint to insert a new room (Add Room)
...
// PUT route to update room status, enable/disable state, and time slot statuses (Enable/Disabled Rooms)
...
app.post("/api/rooms/status/:roomId", (req, res) => {
  ...
  db.query(updateSql, [newStatus, roomId], (error, results) => {
    ...
    if (results.affectedRows > 0) {
      ...
      return res
        .status(404)
        .json({ success: false, error: "No room found or no change needed" });
    }
  });
});
...
app.post("/api/rooms/add", (req, res) => {
  ...
  db.query(query, [roomType, details, img], (err, result) => {
    // ใส่ img ใน array ของ parameters
    if (err) {
...
app.delete("/api/rooms/delete/:roomId", async (req, res) => {
  ...
  try {
    const [roomStatus] = await db.query(
      "SELECT status_8_10, status_10_12, status_13_15, status_15_17 FROM rooms WHERE room_id = ?",
      [roomId]
    );
    const status = roomStatus[0];
    if (
      Object.values(status).every((val) => val === "Free" || val === "Disabled")
    ) {
      ...
      res
        ...
        .json({
          success: false,
          error:
            "Room cannot be deleted unless all time slots are Free or Disabled",
        });
    }
  } catch (error) {
